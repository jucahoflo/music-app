...
export default function PlaylistsPage() {
  ...
  const closePdf = () => setShowPdf(null)
                    ...
                    {songs.map((song, index) => (
                      <tr key={song.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-4">
                          <Link href={`/genres/${song.genreSlug}`} className="inline-block px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs hover:bg-blue-200">
                            {song.genreName}
                          </Link>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            <button onClick={() => playSong(song)} className={`px-3 py-1 rounded text-sm ${currentSong?.id === song.id && isPlaying ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'}`}>
                              {currentSong?.id === song.id && isPlaying ? '⏹' : '▶'}
                            </button>
                            <button onClick={() => viewPdf(song.pdfUrl, song.title)} className="px-3 py-1 bg-gray-600 text-white rounded text-sm">📄</button>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-gray-900 to-gray-800 text-white p-4 shadow-2xl z-50 border-t border-blue-500/30">
          <div className="container mx-auto">
            ...
            <div className="flex justify-between items-center mb-2">
              <div><span className="font-bold text-lg">{currentSong.title}</span><span className="text-gray-400 ml-2">- {currentSong.artist}</span></div>
            ...
            <div className="flex justify-center items-center gap-4 mt-2">
              ...
              <button onClick={() => viewPdf(currentSong.pdfUrl, currentSong.title)} className="bg-purple-600 px-4 py-1.5 rounded-full text-sm">📄 Ver Letra</button>
            </div>
          </div>
        </div>
      )}
      {showPdf && <PdfViewerModal pdfUrl={showPdf.url} title={showPdf.title} onClose={closePdf} />}
    </div>
  )
}
